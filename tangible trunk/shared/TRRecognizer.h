...
public:
    ...
    void touchesBegan(NSSet *touches, UIView *view) {
        std::map<TRIdentifier, TRPoint> map;
        for (UITouch *touch in touches) {
            map[TRIdentifierFromObject(touch)] = [touch locationInView:view];
        }
        touchesBegan(map);
    }
    void touchesMoved(NSSet *touches, UIView *view) {
        std::map<TRIdentifier, TRPoint> map;
        for (UITouch *touch in touches) {
            map[TRIdentifierFromObject(touch)] = [touch locationInView:view];
        }
    ...
    void touchesEnded(NSSet *touches, UIView *view) {
        std::map<TRIdentifier, TRPoint> map;
        for (UITouch *touch in touches) {
            map[TRIdentifierFromObject(touch)] = [touch locationInView:view];
        }
